...
struct Chapter4Story {
    ...
    let kingdomAttacked: Text = Text("The ceremony was well underway when a calamitous explosion erupted from the castle walls, causing panic among the gathered crowd. \(mainCharacter.name), accompanied by their guards, rushed to the scene to confront the unknown threat. As they approached the breach in the walls, a shocking sight awaited them. 4 Hooded figures, one with a \(Villian1Color.auraColor) aura wielding magic of water, the second with \(Villian2Color.auraColor) aura wieding earth, and the third with \(Villian3Color.auraColor) aura wielding lightning all had descended upon the kingdom, attacking its citizens. \(mainCharacter.name) sprang into action, dealing with the intruders one by one, but one of them with a \(Villian4Color.auraColor) aura wielding fire proved to be a formidable adversary. They engaged in a fierce battle that raged on for some time until the hooded figure launched \(mainCharacter.name) into a wall, walking away towards their sinister objective. Though battered and bruised, \(mainCharacter.name) got to their feet and gave chase to the intruder, who was heading toward the castle.")
...
struct Chapter4View: View {
    ...
    @State var goodImage: Bool = false
    @State var badImage: Bool = false
    ...
    var body: some View {
        ...
        if question1{
            OptionView(question: "Do you want to help the boy?", option1: "Help the boy", option2: "Do not interfere", background: "boyWithMonster", decision1: $helpBoy, decision2: $dontHelpBoy,questionBool: $question1)
        }
        if helpBoy{
            ShowStory(textOfStory: story4.helpBoy, heightOfScroll: 900, chapter: 4,storyBool: $helpBoy,questionBool: $question2)
        }
        if dontHelpBoy{
            ShowStory(textOfStory: story4.ignoreBoy, heightOfScroll: 850, chapter: 4,storyBool: $dontHelpBoy,questionBool: $goodCeremony)
        }
        if question2{
            OptionView(question: "Do you want to betray \(mainCharacter.parentalStatus())?", option1: "Betray them", option2: "Stay Calm", background: "KingStaysOrLeaves", decision1: $betrayParent, decision2: $calmDown,questionBool: $question2)
        }
        if betrayParent{
            ShowStory(textOfStory: story4.betrayParent, heightOfScroll: 800, chapter: 4,storyBool: $betrayParent,questionBool: $badCeremony)
        }
        if calmDown{
            ShowStory(textOfStory: story4.calmDown, heightOfScroll: 800, chapter: 4,storyBool: $calmDown,questionBool: $goodCeremony)
        }
        if goodCeremony{
            ShowStory(textOfStory: story4.goodCeremony+story4.kingdomAttacked, heightOfScroll: 1000, chapter: 4,storyBool: $goodCeremony,questionBool: $familyLives)
        }
        if badCeremony{
            ShowStory(textOfStory: story4.badCeremony+story4.kingdomAttacked, heightOfScroll: 1000, chapter: 4,storyBool: $badCeremony,questionBool: $familyDies)
        }
//        if poison{
//            ShowStory(textOfStory: story4.poison, heightOfScroll: 1800, chapter: 4,storyBool: $poison,questionBool: $question3)
//        }
        if helpFamily{
            ShowStory(textOfStory: story4.healFamily, heightOfScroll: 850, chapter: 4,storyBool: $helpBoy,questionBool: $question3)
        }
        if familyLives{
            ShowStory(textOfStory: story4.familyLives, heightOfScroll: 900, chapter: 4,storyBool: $familyLives,questionBool: $question3)
        }
        if familyDies{
            ShowStory(textOfStory: story4.familyDead, heightOfScroll: 850, chapter: 4,storyBool: $familyDies,questionBool: $question3)
        }
        if question3{
            OptionView(question: "Do you want to forgive the mages?", option1: "Forgive them", option2: "Seek Revenge", background: "HoodedOverRuins", decision1: $goodEnding, decision2: $badEnding,questionBool: $question3)
        }
        if goodEnding{
            ShowStory(textOfStory: story4.goodEnding, heightOfScroll: 1350, chapter: 4,storyBool: $goodEnding,questionBool: $goodImage)
        }
        if badEnding{
            ShowStory(textOfStory: story4.badEnding, heightOfScroll: 1850, chapter: 4,storyBool: $badEnding,questionBool: $badImage)
        }
        if goodImage{
            ImageView(background: "WalkIntoDistance2",statement: "The End",pictureBool: $goodImage,storyBool: $continueEnd)
        }
        if badImage{
            ImageView(background: "BurningVillage",statement: "The End",pictureBool: $badImage,storyBool: $continueEnd)
        }
    }
}

struct Chapter4Summary: View{
    var body: some View{
        Text("Summary")
    }
}
